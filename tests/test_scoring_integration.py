...
temp SQLite, exercising all three branches that decide whether/what we log:
  ...
  * no market quote        -> favourite-only paper reco (market_price NULL)
  * quote WITH edge        -> market-priced paper reco (market_price + edge set)
  * quote WITHOUT edge     -> market veto, NOTHING logged (no favourite fallback)

This is the reco-logging path, so it gets a real integration test rather than
trusting the components compose correctly. Predictions-only: no orders are ever
placed — the router reads Polymarket PRICES only.
"""
...
class FakeRouter:
    """Read-only router stub: returns a canned market price quote (or None)."""
    ...
    async def find_best_route(self, home, away, kickoff, outcome):
        ...
        return RoutedQuote(adapter=object(), market=market, quote=self._quote)
...
async def test_market_edge_logs_market_bet(fresh_db, settings):
    # HOME p≈0.98; market price 0.50 -> big positive edge -> reco logged WITH price.
    n = await _score_and_log_one(MATCH, "PL", FakeForm(), StrategyEngine(settings),
...
async def test_exposure_cap_blocks_logging(fresh_db, settings):
    # First reco logs; then force the cap so the next fixture is refused.
