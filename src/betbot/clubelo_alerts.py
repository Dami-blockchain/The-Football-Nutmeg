...
def format_stale_message(status: SnapshotStatus, *, wall_now: datetime) -> str:
    ...
    * missing/unparseable — the club list is empty, so every CL fixture falls
    ...
    return (
        ...
        "The api.clubelo.com CSV/date API is gone for good — ClubElo "
        "deactivated it upstream (their Fixtures API is off and the date "
        "endpoints now 502), so it will not come back. This is not urgent: the "
        "pinned snapshot is still serving CL fine (measured cost only ~1-2% RPS "
        "above a fresh file). A migration onto the site-scale scrape — which "
        "refreshes daily — is being evaluated; if its walk-forward gate passes "
        "it becomes the primary feed and this alarm clears on its own again. "
        "Until then the daily reminder is expected, not a new fault.\n"
